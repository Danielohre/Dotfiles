...
function M.toggle()
	if vim.fn.getbufinfo(terminalBuffer)[1].hidden == 1 then
		...
		terminalWindow = vim.api.nvim_open_win(terminalBuffer, true, window_opts)
	else
...
local function MergeOptions(user_opts)
	for key, val in pairs(default_options) do
		if user_opts[key] ~= nil then
			default_options[key] = user_opts[key]
		else
...
function M.setup(opts)
	...
	vim.api.nvim_set_option_value('winhighlight', 'Normal:TerminalNormal', { win = terminalWindow })
